...
static void changed(GFileMonitor* self, GFile* file, GFile* other_file, GFileMonitorEvent event_type, gpointer user_data) {
    ...
    PathEvents event;
    switch(event_type) {
    case G_FILE_MONITOR_EVENT_CHANGES_DONE_HINT: event = PathEvents::PATH_WRITTEN; break;
    case G_FILE_MONITOR_EVENT_CREATED: event = PathEvents::PATH_CREATED; watcher->reloadMonitors(); break;
    case G_FILE_MONITOR_EVENT_DELETED: event = PathEvents::PATH_DELETED; watcher->reloadMonitors(); break;
    case G_FILE_MONITOR_EVENT_MOVED: case G_FILE_MONITOR_EVENT_RENAMED: event = PathEvents::PATH_MOVED; break;
    default: return;
    }
...
void Util::Path::PathWatcher::startMonitor(std::string path) {
    ...
    if((path == _basePath || _recursive) && type == G_FILE_TYPE_DIRECTORY && _options != WATCH_NONE) {
        ...
        while(true) {
            ...
            if(_options & WATCH_FILE && nextFileType == G_FILE_TYPE_REGULAR) {
                startMonitor(g_file_get_path(nextFile));
            }
            else if(_options & WATCH_FOLDER && nextFileType == G_FILE_TYPE_DIRECTORY) {
                startMonitor(g_file_get_path(nextFile));
            }
        }
